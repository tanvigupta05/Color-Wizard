...
function getRandomColor() {
    return 'hsl(' + ((Math.random() * 30) - 15 + 30) + ',' + (Math.floor(Math.random() * 25) + 50) + '%,' +
    (Math.floor(Math.random() * 20) + 50) + '%)';
}

// Function to apply random background color to divs every 0.5 second
setInterval(() => {
    for (let i = 1; i <= 4; i++) {
...
function copyHexValue() {
    ...
    document.execCommand('copy');
    document.body.removeChild(temp);
    alert('Hex code copied to clipboard: ' + val);
}
